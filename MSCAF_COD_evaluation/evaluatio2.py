...
if __name__=='__main__':
    ...
    data_file = 'txt/HDPNet.txt'
    for i in range(len(datasets)):
        mask_root = '/data/Jenny/Media/COD/' + str(datasets[i]) + '/gt'
        pred_root = '/data/Jenny/2404SAMFree/MSCAF_COD_evaluation/2506compare/' + str(task) + '/' + str(datasets[i]) + '/'
        evaluate_for_datasets(mask_root, pred_root, datasets[i], task, data_file)

    # data_file = 'txt/250613_TTT_abla_mlp.txt'
    # for i in range(len(datasets)):
    #     mask_root = '/data/Jenny/Media/COD/' + str(datasets[i]) + '/gt'
    #     pred_root = '../data/inference_img/' + str(task) + '/' + str(datasets[i]) + '/'
    #     evaluate_for_datasets(mask_root, pred_root, datasets[i], task, data_file)

    # output_file = 'img/241012_ablation.png'
