...
import argparse
import traceback
from model.DWT import extract_high_frequency
import torch.nn as nn

def depthwise_conv_reduce_channels(input_tensor):
    batches, channels, height, width = input_tensor.size()
    depthwise_conv = nn.Conv2d(channels, channels // 3, kernel_size=1, stride=1, padding=0, groups=channels // 48).to(device)
...
parser.add_argument('-i', '--data_path', type=str, default='/dataset/generated/test', help='path to the data folder')
# parser.add_argument('-i', '--data_path', type=str, default='/data/Jenny/MedSAM-main/data/demo2D_vit_b_cod/test', help='path to the data folder')
parser.add_argument('-o', '--seg_path_root', type=str, default='data/inference_npz/SAM-TTT',
                    help='path to the segmentation folder')
parser.add_argument('--seg_png_path', type=str, default='data/inference_img/SAM-TTT',
                    help='path to the segmentation folder')
parser.add_argument('--model_type', type=str, default='vit_b', help='model type')
parser.add_argument('--device', type=str, default='cuda:1', help='device')
parser.add_argument('-chk', '--checkpoint', type=str, default='work_dir_cod/SAM-TTT.pth',
...
def finetune_model_predict(img_np, box_np, boundary, sam_trans, sam_model_tune, device=args.device):
    ...
    with torch.no_grad():
        ...
        dense_embeddings_1, sparse_embeddings_1 = sam_model_tune.ME(dense_embeddings_boundary, dense_embeddings_box,
                                                           high_frequency, sparse_embeddings_box, route=1)
        dense_embeddings_2, sparse_embeddings_2 = sam_model_tune.ME(dense_embeddings_boundary, dense_embeddings_box,
                                                           high_frequency, sparse_embeddings_box, route=2)

        dense_embeddings, sparse_embeddings = sam_model_tune.routefuse(dense_embeddings_1, sparse_embeddings_1,
        ...
        for npz_file in tqdm(npz_files):
            try:
                ...
                for img_id, ori_img in enumerate(ori_imgs):
                    ...
                    seg_mask = finetune_model_predict(ori_img, bbox, bboundary, sam_trans, sam_model_tune, device=device)
                ...
                axes[1].set_title('SAM-TTT: DSC={:.3f}'.format(sam_dice_scores[img_id]))
                axes[1].axis('off')
                # save figure
