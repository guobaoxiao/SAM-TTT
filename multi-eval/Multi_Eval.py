...
def evaluate(opt, args):
    ...
    for dataset in datasets:
        ...
        gt_root = os.path.join(opt.Eval.gt_root, dataset, 'GT')   # COD:GT, polpy:masks
        ...
        for i, sample in samples:
            ...
            if len(pred_mask.shape) != 2:
                pred_mask = pred_mask[:, :, 0]  # 返回(height, width)
            if len(gt_mask.shape) != 2:
                gt_mask = gt_mask[:, :, 0]
            pred_mask = _upsample_like(pred_mask, gt_mask)
